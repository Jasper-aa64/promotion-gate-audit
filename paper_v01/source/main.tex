...
\newcommand{\ArmOrderMeanGap}{5.373}
\newcommand{\ArmOrderMedianGap}{4.733}
\newcommand{\ArmOrderRatio}{1.972}
\newcommand{\CandidateCount}{31}
...
\paragraph{Measurement window.}
The host-weather gate is a preflight: it classifies five control measurements
taken before candidate timing and does not monitor the window that follows.  A
\texttt{QUIET} classification therefore does not establish that the window
stayed quiet.  This study did not detect, record, or isolate interference
arriving after a preflight passed, so any such contribution cannot be separated
from other sources of timing variation.
...
The confounded contrast is not small.  Pooling all \NullPairs{} null pairs,
the candidate-first mean paired difference exceeds the control-first mean by
\ArmOrderMeanGap{} ms, and the medians differ by \ArmOrderMedianGap{} ms:
about \ArmOrderRatio{} times the median practical-effect threshold
$\delta_{\min}$ that the gate itself applies.  Because arm order and parity
are collinear, this quantity cannot be attributed to arm order, to position
within a run, or to any other time-varying cause.  It bounds how much a
design artifact could matter here; it does not show that one did.
...
\paragraph{Pervasive noise flagging.}
The frozen \texttt{noise\_flag} evaluated to \texttt{NOISY} for all
\CandidateCount{} candidates, including the real one.  This is consistent
with a balanced alternating design contributing dispersion to every
candidate, but the study cannot separate such a contribution from host
variation, workload variation, or any other source.  The flag is reported
as a protocol-risk signal only: it does not localize the origin of the
dispersion, and it is not evidence that the frozen judge is inapplicable.
